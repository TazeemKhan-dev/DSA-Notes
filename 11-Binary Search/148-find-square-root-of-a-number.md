...
### Approach 2: Binary Search (l < h Range-Shrinking Style)

**Idea:**
- We shrink the range until low == high, which becomes the floor sqrt.
- Search space: 0 to n.
- We find the smallest mid such that mid*mid > n.
- Then the answer is low - 1.

**Steps:**
- Use l < h
- Check mid*mid
- If mid^2 > n, move left (h = mid)
...
**💭 Intuition Behind the Approach:**
- This approach converges to the first number whose square is > n, then we subtract one.
- We do NOT check for equality; we just narrow the search space until one boundary remains.
